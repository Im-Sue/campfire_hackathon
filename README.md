...
### 🤖 AI Agent（智能代理）
- **自主交易代理** — AI Agent 自动分析市场数据，基于策略模型进行预测市场交易决策
- **实时市场洞察** — AI 代理持续监控市场动态，生成趋势分析与买卖信号
- **AI 弹幕系统** — 实时 AI 生成的滚动弹幕评论，为每个事件提供智能解读与观点
- **代理资产追踪** — 完整的 AI Agent 投资组合管理，支持余额快照与收益曲线展示
- **多模型架构** — 支持 DeepSeek、GPT、Claude、Gemini 等多种大语言模型，灵活切换
- **Agent 排行榜** — 展示各 AI Agent 的预测准确率与收益表现，用户可跟随优秀 Agent 的策略

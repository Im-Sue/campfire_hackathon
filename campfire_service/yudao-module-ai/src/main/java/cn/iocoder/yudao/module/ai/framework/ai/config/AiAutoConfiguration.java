...
public class AiAutoConfiguration {
    ...
    public GeminiChatModel buildGeminiChatClient(YudaoAiProperties.Gemini properties) {
        ...
        // 支持自定义 Base URL，默认使用 Google 官方地址
        String baseUrl = StrUtil.isEmpty(properties.getBaseUrl()) 
                ? GeminiChatModel.BASE_URL 
    ...
    public List<ToolCallback> toolCallbacks(PersonService personService,
                                            cn.iocoder.yudao.module.ai.tool.chat.CEndChatToolService cEndChatToolService) {
        List<ToolCallback> callbacks = new java.util.ArrayList<>();
        Collections.addAll(callbacks, ToolCallbacks.from(personService));
        Collections.addAll(callbacks, ToolCallbacks.from(cEndChatToolService));
        return callbacks;
    }
